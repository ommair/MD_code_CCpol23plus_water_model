...
!         if (induction) then
!            qtx=qtx+ (idmy(js,i)*Etotz(js,i) - idmz(js,i)*Etoty(js,i))
!            qty=qty+ (idmz(js,i)*Etotx(js,i) - idmx(js,i)*Etotz(js,i))
!            qtz=qtz+ (idmx(js,i)*Etoty(js,i) - idmy(js,i)*Etotx(js,i))
!         end if
...
!      write(3001,*)fx(i),fy(i),fz(i)
!      write(4001,*)tx(i),ty(i),tz(i)
...
!         if (induction) then
!            qtx=qtx+ (idmy(js,j)*Etotz(js,j) - idmz(js,j)*Etoty(js,j))
!            qty=qty+ (idmz(js,j)*Etotx(js,j) - idmx(js,j)*Etotz(js,j))
!            qtz=qtz+ (idmx(js,j)*Etoty(js,j) - idmy(js,j)*Etotx(js,j))
!         end if
...
!  update chit to 1/2 step
   chip=chip + 0.5d0*fstep*(3.d0*volmi*(qpres-pres0)/pmass - chip*chit)

!  barostat scale parameter
   scale=exp(-fstep*chip)
...
!   volmi=boxi**3.d0
   ...
   volmi=volmi*exp(3.d0*fstep*chip)

!  update chip to full step

!   boxi = volmi**(1.d0/3.d0)

   boxi=boxi*exp(fstep*chip)
!!   volmi=boxi**3.d0
...
!$omp atomic
   ...
   lstptmp = stptmp
   call system_temp(nm,qtke,qrke,lstptmp)
   stptmp = lstptmp
...
!      write(1111,*) 'force com ', fx(j),fy(j),fz(j), tx(j),ty(j),tz(j)
...
!   volmi = volm0 !boxi**3
   ...
   qpres = (2.d0*tke - qvir)/(3.d0*volmi)
!    qpres = (2.d0*qtke - qvir)/(3.d0*volmi)
!   qpres = (3.d0*boltz*(2.d0*nm-1.d0)*stptmp)/volmi - (qvir)/(3.d0*volmi)
...
!   volm = volmi
   ...
   qpres = (2.d0*tke -qvir)/(3.d0*volm)
!!    qpres = (3.d0*boltz*(2.d0*nm-1.d0)*stptmp)/volm - (qvir)/(3.d0*volm)
