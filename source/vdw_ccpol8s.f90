module vdw_ccpol8s
  ...
  public :: ccpol8s, ccpol8s_lrc

contains

  subroutine ccpol8s(xx,yy,zz,xxs,yys,zzs,boxi,qqvdwe)
...
!	write(dbgUnit, *) 'P#', MPIrank, '. ccpol8s calc time	', deltaTime;
	! DBG
...
! MPI sync
     ...
     lvdwlrc = vdwlrc
     lvirlrc = virlrc
     call ccpol8s_lrc(nm,box,rcut,lvdwlrc,lvirlrc)
...
!     write(*,*) 'vdw: ', vir,vir_test,qvir_vdw
	...
	return;
  end subroutine ccpol8s

  subroutine ccpol8s_lrc(n,bx,rc,ulrc,wlrc)
...
!$omp parallel DEFAULT(SHARED)&
!$omp& private(m,uedis,uele,uin6,uin8,uin10,wedis,wele,win6,win8,win10)&
...
!$omp end parallel
  ...
  end subroutine ccpol8s_lrc 

end module vdw_ccpol8s 
