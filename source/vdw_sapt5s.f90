module vdw_sapt5s
  ...
  public :: sapt5s, sapt5s_lrc

contains

  subroutine sapt5s(xx,yy,zz,xxs,yys,zzs,boxi,qvdwe)
...
!             Sapt5s Energy, Virial and Forces                   
                          ...
                          qqvdwe = qqvdwe + u_exp+u_ele+u_ind
                          ...
                          ! derivative of asymptotic part of the dispersion
                          ! interaction (du_ind/drij)
                          df6=dtt(6, d6(k), sqr)
                          df8=dtt(8, d8(k), sqr)
...
!             Coulombic Energy, Virial and Force (real space Ewald sum term)  
                       
!                          call rspace_ewald(qis,qjs,rssq,alpha,qrcpe,cfij)  
!              Accumalating focces
      
                          fxs(is,i)=fxs(is,i)+rsx*vfij
                          fys(is,i)=fys(is,i)+rsy*vfij
                          fzs(is,i)=fzs(is,i)+rsz*vfij

                          fxs(js,j)=fxs(js,j)-rsx*vfij
                          fys(js,j)=fys(js,j)-rsy*vfij
                          fzs(js,j)=fzs(js,j)-rsz*vfij
...
!	write(dbgUnit, *) 'P#', MPIrank, '. sapt5s calc time	', deltaTime;
	! DBG
...
! MPI sync
     ...
     lvdwlrc = vdwlrc
     lvirlrc = virlrc
     call sapt5s_lrc(nm,box,rcut,lvdwlrc,lvirlrc)
     vdwlrc = lvdwlrc/418.4d0
     virlrc = lvirlrc/418.4d0

     qvdwe = qqvdwe/418.4d0

     qvdwe = qvdwe !+ vdwlrc 
 
	return;
  end subroutine sapt5s

  subroutine sapt5s_lrc(n,bx,rc,ulrc,wlrc)
...
! corrections
  ...
  end subroutine sapt5s_lrc 

end module vdw_sapt5s 
